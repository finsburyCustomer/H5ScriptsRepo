...
class H5PMS100B_PMS050A  {
    ...
    public static Init(args: IScriptArgs): void {
        new H5PMS100B_PMS050A (args).run();
    }
    ...
    private run(): void {
        ...
        var top = 8;
        var left = (ScriptUtil.FindChild($(".lawsonHost:visible"), "contentBody").width() / 2 - 320) / 10;
        var eid = "h5btn_morcpt" + Math.floor(Math.random() * 10000);
        const buttonElement = new ButtonElement();
        buttonElement.Name = eid;
        buttonElement.Value = "MO Rcpt";
    ...
    private OnClickButton() {
        ...
        if (validFields) {
            for (var i = 0; i < rows.length; i++) {
                ...
                auto.addStep(ActionType.Run, "PMS050");
                auto.addStep(ActionType.Set, "");
                auto.addField("WHFACI", FACI);
                auto.addField("WHPRNO", PRNO);
                auto.addField("WHMFNO", MFNO);
